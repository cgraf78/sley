...
# The shared bootstrap and link publisher remain compatible with macOS system
# Bash 3.2, but the published command uses Bash 4 features. Check the executable
# that `#!/usr/bin/env bash` will resolve before creating any directories or
# links, so a successful install cannot publish a command that immediately dies.
RUNTIME_BASH=$(type -P bash) ||
  die 'Bash 4 or newer must be first on PATH'
RUNTIME_BASH_MAJOR=$(
  # shellcheck disable=SC2016 # Expanded by the PATH-selected child Bash.
  "$RUNTIME_BASH" -c 'printf "%s\n" "${BASH_VERSINFO[0]}"' 2>/dev/null
) || RUNTIME_BASH_MAJOR=
if [[ ! "$RUNTIME_BASH_MAJOR" =~ ^[0-9]+$ ||
  "$RUNTIME_BASH_MAJOR" -lt 4 ]]; then
  die 'Bash 4 or newer must be first on PATH'
fi
  ...
  for ((index = count - 1; index >= 0; index--)); do
    ...
    if [[ "${prior_states[$index]}" == symlink ]]; then
      if ! ln -sfn -- "${prior_targets[$index]}" "$target"; then
        printf 'sley: rollback failed to restore symlink: %s\n' "$target" >&2
      fi
    elif ! rm -f "$target"; then
      printf 'sley: rollback failed to remove new link: %s\n' "$target" >&2
    fi
  done
...
printf 'installed sley commands to %s\n' "$BIN_DIR"
printf 'installed sley manpages to %s\n' "$MAN_DIR"
